...
def invoke_get_ports(station_id: str) -> None:
    client = boto3.client("lambda", region_name=AWS_REGION)

    payload = {
        "service": {"action": "getPortsByStation", "callerId": "script"},
    ...
    ports = response_json["data"]["ports"]
    assert isinstance(ports, list), "ports must be a list"
    print(json.dumps(response_json, indent=2))
    print(f"Retrieved {len(ports)} port(s) for station {station_id}.")


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage: python -m tests.integration.read.get_ports_dynamo_invoker <stationId>"
        )
        sys.exit(1)

    invoke_get_ports(sys.argv[1])